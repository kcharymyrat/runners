...
func (rr RunnersRepository) UpdateRunner(runner *models.Runner) *models.ResponseError {
	...
	SET first_name = $1, last_name = $2, age = $3, country = $4
	WHERE id = $5`
...
func (rr RunnersRepository) UpdateRunnerResults(runner *models.Runner) *models.ResponseError {
	query := `UPDATE runners
	SET personal_best = $1, seasonal_best = $2
	WHERE id = $3`

	_, err := rr.transaction.Exec(query, runner.PersonalBest, runner.SeasonBest)
	if err != nil {
		return &models.ResponseError{
			Message: err.Error(),
			Status:  http.StatusInternalServerError,
...
func (rr RunnersRepository) GetAllRunners() ([]*models.Runner, *models.ResponseError) {
	query := `SELECT * FROM runners WHERE is_active = 'true'`
...
func (rr RunnersRepository) GetRunnersByCountry(ctry string) ([]*models.Runner, *models.ResponseError) {
	query := `SELECT * FROM runners
	WHERE country = $1 AND is_active = 'true'
	ORDER BY personal_best
	LIMIT 10`

	rows, err := rr.dbHandler.Query(query, ctry)
	if err != nil {
		return nil, &models.ResponseError{
			Message: err.Error(),
			Status:  http.StatusInternalServerError,
...
func (rr RunnersRepository) GetRunnersByYear(year int) ([]*models.Runner, *models.ResponseError) {
	query := `SELECT runners.id, runners.first_name,
	runners.last_name, runners.age, runners.is_active,
	runners.country, runners.personal_best,
	results.race_result 
	FROM runners
	INNER JOIN results ON runners.id = results.runner_id
	WHERE results.year = $1
	ORDER BY runners.season_best
	LIMIT 10`
