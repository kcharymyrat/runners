...
func (rr ResultsRepository) CreateResult(result *models.Result) (*models.Result, *models.ResponseError) {
	query := `INSERT INTO results(runner_id, race_result, location, position, year)
	VALUES ($1, $2, $3, $4, $5)
	RETURNING id`

	rows, err := rr.dbHandler.Query(query, result.ID, result.RaceResult, result.Location, result.Position, result.Year)
	if err != nil {
